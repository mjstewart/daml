...
data ScenarioF expr
  ...
  | SCommitAbortMsgF !Type !expr !expr !expr
  | SMustFailAtMsgF  !Type !expr !expr !expr
  | SPassF           !expr
  | SGetTimeF
  | SGetPartyF       !expr
...
projectScenario = \case
  ...
  SCommitAbortMsg a b c d -> SCommitAbortMsgF a b c d
  SMustFailAtMsg a b c d -> SMustFailAtMsgF a b c d
  SPass a -> SPassF a
  SGetTime -> SGetTimeF
...
embedScenario = \case
  ...
  SCommitAbortMsgF a b c d -> SCommitAbortMsg a b c d
  SMustFailAtMsgF a b c d -> SMustFailAtMsg a b c d
  SPassF a -> SPass a
  SGetTimeF -> SGetTime
